...
Our agent is trained on 3.5 years of historical 30-minute OHLC data for 11 Deribit perpetuals. On the 3-month hold-out test set, the agent easily outperforms benchmarks, achieving 59% ROI / 2.15 Sharpe, vs. -39% / -2.11 (uniform buy-and-hold) and 7.3% / 1.28 (best individual asset).

Furthermore, we analyze agent training dynamics, diagnose four failure patterns (zero cash allocation, exploitation of high volatility, low portfolio diversification and high turnover) and implement targeted fixes, including an architectural change to restore cash allocation and a per-asset liquidity penalty to reduce turnover.
